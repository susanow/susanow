...
void l2fwd(void*)
{
  ...
  while (running) {
    for (size_t pid=0; pid<nb_ports; pid++) {
      ...
      size_t nb_enq = ring_rx[pid][pivot]->enq_bulk((void**)mbufs, nb_recv);
      if (nb_recv > nb_enq) {
    ...
    for (size_t pid=0; pid<nb_ports; pid++) {
      rte_mbuf* mbufs[32];
      size_t nb_deq = ring_tx[pid][pivot]->deq_bulk((void**)mbufs, 32);
...
void wk0(void*)
{
  ...
  while (running) {
    for (size_t pid=0; pid<nb_ports; pid++) {
      rte_mbuf* mbufs[32];
      size_t nb_deq = ring_rx[pid][0]->deq_bulk((void**)mbufs, 32);
...
void wk1(void*)
{
  ...
  while (running) {
    for (size_t pid=0; pid<nb_ports; pid++) {
      rte_mbuf* mbufs[32];
      size_t nb_deq = ring_rx[pid][1]->deq_bulk((void**)mbufs, 32);
      if (nb_deq == 0) continue;
      for (size_t i=0; i<nb_deq; i++) {
